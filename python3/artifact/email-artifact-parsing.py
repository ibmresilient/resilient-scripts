...
def storeHashes(the_data: bytes, the_description: str):
    ...
    incident.addArtifact("Malware MD5 Hash", hashlib.md5(
        the_data).hexdigest(), "MD5 {}".format(the_description))
    incident.addArtifact("Malware SHA-1 Hash", hashlib.sha1(
        the_data).hexdigest(), "SHA1 {}".format(the_description))
    incident.addArtifact("Malware SHA-256 Hash", hashlib.sha256(
        the_data).hexdigest(), "SHA256 {}".format(the_description))


headers = Parser(policy=default).parsestr(artifact.value)

# Extract the fundamental aspects of the email as artifacts
incident.addArtifact("Email Recipient", '{}'.format(
    headers['to']), "Email Recipient")
incident.addArtifact("Email Sender", '{}'.format(
    headers["from"]), "Email Sender")
incident.addArtifact("Email Subject", '{}'.format(
    headers['subject']), "Email Subject")
incident.addArtifact("Email Recipient", '{}'.format(
    headers['to'].addresses[0].username), "Email Recipient username")
incident.addArtifact("Email Sender Name",
                     headers['from'].addresses[0].display_name, "Email Sender")

# Process the body of the email message
...
if the_body['content-type'].maintype == 'text':
    ...
    for part in body.iter_attachments():
        incident.addArtifact("Email Attachment Name", part.get_filename(
        ), "Suspicious email attachment name")
