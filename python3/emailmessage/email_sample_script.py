    ...
    def checkIsItemNotOnWhiteList(self, anItem):
        ...
        for whiteListEntry in self:
            if whiteListEntry.test(anItem):
                log.info(u"Filtering out {0} because it matched with whitelist entry {1}".format(
                    anItem, self))
                return None
        return anItem
...
class EmailProcessor(object):
    ...
    ipV6WhiteListConverted = WhiteList.createFromCollection(
        ipV6WhiteList, CIDR)

    # Create Whitelist domains from string representations
    domainWhiteListConverted = WhiteList.createFromCollection(
    ...
    def checkIPWhiteList(self, anAddress):
        ...
        log.debug(u"Going to filter {0} against whitelist {1}".format(
            anAddress, whiteList))
    ...
    def checkDomainWhiteList(self, aURL):
        ...
        log.debug(u"Going to filter {0} against whitelist {1}".format(
            aURL, self.domainWhiteListConverted))
    ...
    def processIPFully(self, theAddressAsString):
        ...
        if theAddressAsString is not None:
            ...
            if theAddressAsObj is not None:
                theAddressAsObj = self.checkIPWhiteList(
                    theAddressAsObj)  # Check against whitelist
                if theAddressAsObj is not None:
    ...
    def addBasicInfoToIncident(self):
        ...
        if hasattr(emailmessage.sender, "name"):
...
), "URL", "Suspicious URL", processor.fixURL, processor.checkDomainWhiteList)
